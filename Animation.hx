...
class Animation extends DisplayObjectContainer implements IAnimatable
{
    ...
    {
        currentFrame = Std.isOfType(indexOrLabel, String) ? _symbol.getFrame(cast(indexOrLabel, String)) : Std.int(indexOrLabel);
    }
    ...
    {
        var frameIndex:Int = Std.isOfType(indexOrLabel, String) ? _symbol.getFrame(cast(indexOrLabel,String)) : Std.int(indexOrLabel);
    ...
    public function setSymbol(symbol:Symbol):Void {
        _symbol.dispose();
        if (_symbol.filter != null) _symbol.filter.dispose();
        _symbol = symbol;
    }
}
