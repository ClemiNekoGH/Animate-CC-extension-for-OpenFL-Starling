...
class MovieBehavior extends EventDispatcher implements IAnimatable {
    ...
    public function new(target:DisplayObject, onFrameChanged:Function, frameRate:Float=24) {
        super();
    ...
    public function advanceTime(passedTime:Float):Void {
        ...
        if (previousFrameID != _currentFrame) _onFrameChanged(_currentFrame);
    ...
    private function set_numFrames(value:Int):Int {
        _frames = [];
        for (i in numFrames...value) _frames[i] = new MovieFrame();
        return value;
    }
    ...
    private function set_currentTime(value:Float):Float {
        ...
        if (prevFrame != _currentFrame) _onFrameChanged(_currentFrame);
        return value;
    }
    ...
    private function get_isPlaying():Bool {
        if (_playing) return _loop || _currentTime < totalTime;
        else return false;
    }
...
class MovieFrame {
    ...
    public function addAction(action:Function):Void {
        ...
        if (Lambda.indexOf(_actions, action) == -1) _actions[_actions.length] = action;
    }

    public function removeAction(action:Function):Void {
        if (_actions != null) {
            var index:Int = Lambda.indexOf(_actions, action);
            if (index >= 0) _actions.splice(index, 1)[0];
        }
    }
